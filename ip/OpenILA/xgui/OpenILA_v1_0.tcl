...
proc init_gui { IPINST } {
  ...
  ipgui::add_param $IPINST -name "G_UART_ACTIVE" -parent ${Page_0}
  ipgui::add_param $IPINST -name "G_UART_BAUDS" -parent ${Page_0} -widget comboBox
...
proc update_PARAM_VALUE.G_UART_BAUDS { PARAM_VALUE.G_UART_BAUDS PARAM_VALUE.G_UART_ACTIVE } {
	# Procedure called to update G_UART_BAUDS when any of the dependent parameters in the arguments change
	
	set G_UART_BAUDS ${PARAM_VALUE.G_UART_BAUDS}
	set G_UART_ACTIVE ${PARAM_VALUE.G_UART_ACTIVE}
	set values(G_UART_ACTIVE) [get_property value $G_UART_ACTIVE]
	if { [gen_USERPARAMETER_G_UART_BAUDS_ENABLEMENT $values(G_UART_ACTIVE)] } {
		set_property enabled true $G_UART_BAUDS
	} else {
		set_property enabled false $G_UART_BAUDS
	}
}

proc validate_PARAM_VALUE.G_UART_BAUDS { PARAM_VALUE.G_UART_BAUDS } {
	# Procedure called to validate G_UART_BAUDS
	return true
}
...
proc update_PARAM_VALUE.G_UART_ACTIVE { PARAM_VALUE.G_UART_ACTIVE } {
	# Procedure called to update G_UART_ACTIVE when any of the dependent parameters in the arguments change
}

proc validate_PARAM_VALUE.G_UART_ACTIVE { PARAM_VALUE.G_UART_ACTIVE } {
	# Procedure called to validate G_UART_ACTIVE
	return true
}
...
proc update_MODELPARAM_VALUE.G_UART_ACTIVE { MODELPARAM_VALUE.G_UART_ACTIVE PARAM_VALUE.G_UART_ACTIVE } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.G_UART_ACTIVE}] ${MODELPARAM_VALUE.G_UART_ACTIVE}
}

proc update_MODELPARAM_VALUE.G_UART_BAUDS { MODELPARAM_VALUE.G_UART_BAUDS PARAM_VALUE.G_UART_BAUDS } {
	# Procedure called to set VHDL generic/Verilog parameter value(s) based on TCL parameter value
	set_property value [get_property value ${PARAM_VALUE.G_UART_BAUDS}] ${MODELPARAM_VALUE.G_UART_BAUDS}
}
